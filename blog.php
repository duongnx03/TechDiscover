...
if (isset($_POST['search'])) {
    ...
    $blogs = $blog->searchBlogsByTitle($searchTerm);
} elseif (isset($_GET['category'])) {
    // Nếu có danh mục được chọn, hiển thị blog của danh mục đó
    $category_id = $_GET['category'];
    $blogs = $blog->getBlogsByCategory($category_id, $limit);
} else {
                ...
                <ul class="pagination justify-content-center">
                    <?php for ($i = 1; $i <= $totalPages; $i++) : ?>
                        <li class="page-item <?= ($page == $i) ? 'active' : ''; ?>">
                            <a class="page-link" href="?page=<?= $i; ?><?php if (isset($_GET['category'])) echo "&category=" . $_GET['category']; ?>"><?= $i; ?></a>
                        </li>
                    <?php endfor; ?>
                </ul>
            </nav>
